...
A follow-up pixel comparison showed that Windows 11 can translate the XAML notification controls 20–21 pixels to the left while the `TrayNotifyWnd` rectangle and Q-Tracker rectangle remain unchanged. The original 12-pixel gap therefore did not protect the full interactive area in every layout state. The safe gap must scale with taskbar height so it reserves at least one taskbar control slot rather than relying on the HWND boundary alone.
...
4. For a horizontal taskbar, set the overlay's maximum right edge to `TrayNotifyWnd.left - max(12, taskbar_height)` pixels. This reserves one DPI-scaled taskbar control slot for XAML content that can extend left of the HWND boundary.
...
- A valid notification area moves a 400-pixel overlay so its right edge is at least one taskbar-height slot before `TrayNotifyWnd.left` (48 pixels on the active taskbar).
- A 64-pixel DPI-scaled taskbar receives a 64-pixel notification-area gap.
